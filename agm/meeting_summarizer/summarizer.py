# Vendored unchanged from supertypeai/summarize-agm-result@cfc119c (src/meeting_summarizer/summarizer.py)
...
Use natural language with complete sentences and proper punctuation.
...
async def _summarize_chunks_async(
    summarize_chunk: Callable[[httpx.AsyncClient, str, int, int], Awaitable[str]],
    chunks: list[str],
) -> list[str]:
    semaphore = asyncio.Semaphore(_CHUNK_CONCURRENCY)
    async with httpx.AsyncClient(timeout=240) as client:
        chunk_tasks = [
            _summarize_chunk_with_limit(
                semaphore,
                summarize_chunk,
                client,
                chunk,
        ...
        return list(await asyncio.gather(*chunk_tasks))


async def _summarize_chunk_with_limit(
    ...
    async with semaphore:
        return await summarize_chunk(client, chunk, index, total_chunks)
...
class MeetingSummarizer:
    ...
    def summarize(
        ...
        if self.settings.api == "openrouter":
            if normalized_doc_type == "agms":
                summarize_chunk_async = self._summarize_chunk_openrouter_async
            else:
                summarize_chunk_async = self._summarize_chunk_pubex_openrouter_async
            partials = asyncio.run(
                _summarize_chunks_async(summarize_chunk_async, chunks)
            )
        else:
            partials = []
            for index, chunk in enumerate(chunks, start=1):
...
{chunk}
""".strip()
        chat_request = chat or self._chat
        return chat_request(prompt)

    async def _summarize_chunk_openrouter_async(
        self,
        client: httpx.AsyncClient,
        chunk: str,
        index: int,
        ...
        chat_request = self._summarize_chunk(
            chunk,
            index,
            total,
            partial(self._chat_openrouter_async, client),
        )
...
""".strip()
    ...
    async def _summarize_chunk_pubex_openrouter_async(
        self,
        client: httpx.AsyncClient,
        chunk: str,
        index: int,
        ...
        chat_request = self._summarize_chunk_pubex(
            chunk,
            index,
            total,
            partial(self._chat_openrouter_async, client),
...
""".strip()
    ...
    def _chat_openrouter(self, prompt: str) -> str:
        ...
        payload = {
            "model": f"{self.settings.model}:nitro",
        ...
        if response.status_code >= 400:
            ...
            raise RuntimeError(
                f"OpenRouter API request failed: {response.status_code} {error_body}"
            )
