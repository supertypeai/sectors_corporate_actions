# Vendored unchanged from supertypeai/summarize-agm-result@cfc119c (src/meeting_summarizer/config.py)
...
ApiProvider = Literal["openai", "gemini", "openrouter"]
...
def load_settings(api: ApiProvider) -> Settings:
    if api == "openai":
        ...
        if not api_key:
            raise RuntimeError("OPENROUTER_API_KEY is required when --api openrouter is used.")
